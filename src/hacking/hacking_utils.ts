...
export function attackServer(ns: NS, target: ServerData, workers: ServerData[]) {
  const attack_data = new ServerHackAnalysis(ns, target, 0.1);
  // handle weaken first
...
function getExploitArg(target: string, delay: number, sleep_time: number) {
  return ['--target', target, '--delay', delay, '--sleep', sleep_time, '--id', id++];
}

function getAvailibleRam(ns: NS, server: ServerData) {
  return server.ram - ns.getServerUsedRam(server.name);
}

function getAvailibleThreads(ns: NS, server: ServerData, script_ram: number) {
